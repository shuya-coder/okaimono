...
スマートフォンで買い物中に片手操作しやすいよう、商品追加、リスト確認、カート移動、会計済み処理をシンプルにまとめています。
...
- リストの商品カードから「カート」「＋」「－」「削除」を一列で操作
- カートの商品カードから「戻す」「＋」「－」「削除」を一列で操作
- カートの商品を「戻す」でリストへ戻す
- カートの「会計済み」でカート内商品を一括削除し、履歴へ保存
- 履歴を日付ごとにグループ表示
...
- `ShoppingManager` がリストとカートの商品状態を管理します。
